...
// The `Rule` enum is generated by pest_derive from `expr.pest` and cannot carry
// doc comments, so exempt this module from `missing_docs`.
#![allow(missing_docs)]
...
/// Parser for MBA expressions and programs, generated from `expr.pest`.
#[derive(Parser)]
#[grammar = "expr.pest"]
                ...
                Rule::unknown => {
                    ...
                    if let Some(vars_pair) = inner.next() {
                        for v in vars_pair.into_inner() {
                            let v_id: usize = v.as_str()[1..].parse().map_err(|_| {
                                format!("variable index out of range: {}", v.as_str())
                            })?;
        ...
        assert_eq!(
            *program.get_index(4).unwrap(),
            Insn {
                ty: 8,
                id: 4.into(),
                kind: InsnKind::Assign(Expr::Var(2.into()) + Expr::Const(2) * Expr::Var(3.into()))
            }
        );
        assert_eq!(
